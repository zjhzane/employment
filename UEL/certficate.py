...
def main():
    ...
    parser = argparse.ArgumentParser(description="给目标 PDF 添加印章（叠加图片并导出新 PDF）")
    ...
    parser.add_argument(
        ...
        default=str(desktop / "zxzmywdy.pdf"),

        help="目标 PDF 路径（默认桌面 zxzmywdy.pdf）",
    ...
    parser.add_argument(
        ...
        help="输出 PDF 路径；不填则输出到桌面：certficate.pdf",
    ...
    parser.add_argument("--stamp_y_ratio", type=float, default=0.8, help="章左上角相对高度比例（越大越靠下）")

    parser.add_argument("--stamp_w_ratio", type=float, default=0.23, help="章宽度相对整图宽度比例")
    ...
    out_pdf = Path(args.out) if args.out else desktop / "certficate.pdf"
